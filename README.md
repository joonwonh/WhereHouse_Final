...
WHERE HOUSE는 서울시 거주를 희망하는 20대들에게 거주지 선정에 대한 정보를 제공하는 동시에<br> 본인이 희망하는 조건의 거주지를 추천해주기 위한 서비스입니다.
...
- 추천 기준 우선 순위
  1. 가격
  2. 편의성과 안전성의 중요도
- 추천된 해당 구의 평균 가격과 안전, 편의 점수를 보여주고, <br>
  전체 지역구의 편의, 안전 관련 평균 정보와 해당 지역구의 편의 안전 정보를 chart.js로 비교를 해줍니다.<br>(비동기로 데이터를 받아오기 위해 Ajax 사용)<br>
  또한, 해당 지역구의 총평도 확인할 수 있습니다.
  우측 지도 화면에서는(KakaoMap api) 추천된 3개의 지역구를 인구밀집도 단계에 맞는 색상으로 표시를 해줍니다.
...
- 추천된 3개의 지역구들 중 비교해보고싶은 지역구를 선택한 뒤, 상세 비교 버튼을 누르게 되면,<br>
  선택된 지역구끼리의 안전시설, 편의시설의 정보들을 비교해볼 수 있는 모달창을 띄워줍니다.
...
서울시 25개 구의 가격, 편의, 안전, 핫플레이스, 정보 별 순위 등을 확인해 볼 수 있는 지역구 정보 페이지입니다.
- 정보를 확인해보고 싶은 지역구를 지도 화면에서 선택하거나 셀렉트바에서 선택하게 되면,<br>
  해당 지역구의 평균 전/월세가, 안전, 편의 점수 그리고 핫플레이스 등의 정보들을 확인할 수 있습니다.
- 지도화면에서 지역구를 선택했다면, 해당 지역구와 서울시 평균을 비교하여 선정해놓은 순위 정보를 확인할 수 있습니다.
...
사용자들간의 정보들을 주고받을 수 있는 게시판 페이지입니다.
- 작성되어 있는 게시글을 읽어볼 수 있습니다.
  해당 게시글에 댓글을 남길수가 있는데, 만약 로그인이 되어있지 않다면 로그인 화면으로 넘어가게 됩니다.
- 로그인이 되어있는 상태라면, 글 작성하러 가기! 버튼을 클릭하여 게시글을 작성할 수 있습니다.
- 게시글 중 본인이 작성한 게시글 또는 댓글은 수정, 삭제가 가능합니다.
